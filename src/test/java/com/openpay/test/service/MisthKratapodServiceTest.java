...
public class MisthKratapodServiceTest {
  ...
  public static void setUp() {
    ...
    // test data
    when(serviceMock.find(records[0].getKodkratapod())).thenReturn(records[0]);
  }

  @Test
  public void testFind_success() {
    MisthKratapod testResult = serviceMock.find(records[0].getKodkratapod());

    org.junit.Assert.assertNotNull(testResult);
    org.junit.Assert.assertEquals(
        "expect equals kodxrisi ", records[0].getKodxrisi(), testResult.getKodxrisi());
    org.junit.Assert.assertEquals(
        "expect equals desckratapod ", records[0].getDesckratapod(), testResult.getDesckratapod());
  }
}
