...
import com.openpay.test.dao.DefaultMisthKratapodDao;
...
public class MisthKratapodHandlerTestIt {
  static final String inputFile = "MisthKratapod.json";
  static MisthKratapodHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthKratapod.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthKratapod[].class);
    assertEquals("match count", count, records.length);
    MisthKratapod testResult = jpa.find(MisthKratapod.class, records[0].getKodkratapod());
    assertEquals(
