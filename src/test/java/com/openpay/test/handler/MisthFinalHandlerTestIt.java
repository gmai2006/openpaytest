...
import com.openpay.test.dao.DefaultMisthFinalDao;
...
public class MisthFinalHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthFinal.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthFinal[].class);
    assertEquals("match count", count, records.length);
    final MisthFinalId id =
        new MisthFinalId(this.records[0].getKodfinal(), this.records[0].getKodxrisi());
    MisthFinal testResult = jpa.find(MisthFinal.class, id);
    assertEquals(
