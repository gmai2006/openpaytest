...
import com.openpay.test.dao.DefaultMisthFyloDao;
...
import com.openpay.test.entity.MisthFyloId;
...
public class MisthFyloHandlerTestIt {
  static final String inputFile = "MisthFylo.json";
  static MisthFyloHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthFylo.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthFylo[].class);
    assertEquals("match count", count, records.length);
    final MisthFyloId id =
        new MisthFyloId(this.records[0].getKodfylo(), this.records[0].getKodxrisi());
    MisthFylo testResult = jpa.find(MisthFylo.class, id);
    assertEquals(
