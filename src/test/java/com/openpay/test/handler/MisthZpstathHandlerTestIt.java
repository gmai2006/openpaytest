...
import com.openpay.test.dao.DefaultMisthZpstathDao;
...
public class MisthZpstathHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthZpstath.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthZpstath[].class);
    assertEquals("match count", count, records.length);
    final MisthZpstathId id =
        new MisthZpstathId(this.records[0].getKodxrisi(), this.records[0].getKodstath());
    MisthZpstath testResult = jpa.find(MisthZpstath.class, id);
    assertEquals(
