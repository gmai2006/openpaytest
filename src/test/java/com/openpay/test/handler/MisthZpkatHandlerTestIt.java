...
import com.openpay.test.dao.DefaultMisthZpkatDao;
...
import com.openpay.test.entity.MisthZpkatId;
...
public class MisthZpkatHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthZpkat.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthZpkat[].class);
    assertEquals("match count", count, records.length);
    final MisthZpkatId id =
        new MisthZpkatId(this.records[0].getKodxrisi(), this.records[0].getKodkat());
    MisthZpkat testResult = jpa.find(MisthZpkat.class, id);
