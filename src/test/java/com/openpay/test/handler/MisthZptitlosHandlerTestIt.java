...
import com.openpay.test.dao.DefaultMisthZptitlosDao;
...
public class MisthZptitlosHandlerTestIt {
  static final String inputFile = "MisthZptitlos.json";
  static MisthZptitlosHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthZptitlos.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthZptitlos[].class);
    assertEquals("match count", count, records.length);
    final MisthZptitlosId id =
        new MisthZptitlosId(this.records[0].getKodxrisi(), this.records[0].getKodtitlos());
    MisthZptitlos testResult = jpa.find(MisthZptitlos.class, id);
    assertEquals(
