...
import com.openpay.test.dao.DefaultMisthYpalYvarDao;
...
public class MisthYpalYvarHandlerTestIt {
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthYpalYvar.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthYpalYvar[].class);
    assertEquals("match count", count, records.length);
    final MisthYpalYvarId id =
        new MisthYpalYvarId(
            this.records[0].getKodyvar(),
            this.records[0].getKodxrisi(),
            this.records[0].getKodypal());
    MisthYpalYvar testResult = jpa.find(MisthYpalYvar.class, id);
