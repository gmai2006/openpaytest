...
import com.openpay.test.dao.DefaultMisthZptmimaDao;
...
public class MisthZptmimaHandlerTestIt {
  static final String inputFile = "MisthZptmima.json";
  static MisthZptmimaHandler handler;
  ...
  public void testSelect() throws IOException {
    final File tempFile = new File("./src/test/resources/MisthZptmima.csv");
    final InputStream inputStream = new BufferedInputStream(new FileInputStream(tempFile));
    int count = handler.process(inputStream);
    String json = FileUtils.readFileFromResource2String(inputFile, Charset.defaultCharset());
    records = gson.fromJson(json, MisthZptmima[].class);
    assertEquals("match count", count, records.length);
    final MisthZptmimaId id =
        new MisthZptmimaId(this.records[0].getKodxrisi(), this.records[0].getKodtmima());
    MisthZptmima testResult = jpa.find(MisthZptmima.class, id);
    assertEquals(
