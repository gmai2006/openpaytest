...
// @Stateless
@Named("MisthKratapodHandler")
public class MisthKratapodHandler extends DelimiterFileHandler<MisthKratapod> {

  @Inject
  @Named("DefaultJpaDao")
  public MisthKratapodHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected MisthKratapod parseLine(List<String> headers, List<String> tokens) {
    MisthKratapod record = new MisthKratapod();
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        case "kodkratapod":
          record.setKodkratapod(java.lang.Long.valueOf((tokens.get(i))));
          break;
        case "kodxrisi":
