...
// @Stateless
@Named("MisthZpstathHandler")
public class MisthZpstathHandler extends DelimiterFileHandler<MisthZpstath> {

  @Inject
  @Named("DefaultJpaDao")
  public MisthZpstathHandler(final JpaDao dao) {
    super(dao);
  }

  @Override
  protected MisthZpstath parseLine(List<String> headers, List<String> tokens) {
    ...
    for (int i = 0; i < tokens.size(); i++) {
      switch (headers.get(i)) {
        ...
        case "poso":
          record.setPoso(java.lang.Double.valueOf((tokens.get(i))));
          break;
